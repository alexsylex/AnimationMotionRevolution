...
namespace AMR
{
	...
	// static
	std::uint32_t __fastcall hkbClipGenerator::sub_140A0F480_Hook(RE::hkbClipGenerator* a_this)
	{
			...
			if(RE::hkbContext* context = GethkbContext())
			{
				...
				else
				{
					...
					for(RE::hkaAnnotationTrack& annotationTrack : a_this->binding->animation->annotationTracks)
					{
						...
						{
							std::sort(characterMotionMap[character].motionList.begin(), characterMotionMap[character].motionList.end(),
									  [](const MotionData& a_lhs, const MotionData& a_rhs) -> bool
									  {
	...
	// static
	void __fastcall MotionPositionContainer::sub_1404DD5A0_Hook(RE::MotionPositionContainer* a_this, float a_motionTime, RE::NiPoint3* a_pos)
	{
		...
		if(characterMotionMap.contains(character))
		{
			...
			for(unsigned int segIndex = 1; segIndex <= motionList.size(); segIndex++)
			{
				...
				if(curMotionTime <= curSegMotionTime)
				{
					...
					const RE::NiPoint3& curSegMotionPos = motionList.at(segIndex - 1).pos;
					const RE::NiPoint3& prevSegMotionPos = prevSegIndex?
						motionList.at(prevSegIndex - 1).pos :
						RE::NiPoint3{ 0.0f, 0.0f, 0.0f };

					*a_pos = curSegMotionPos * segProgress + prevSegMotionPos * (1.0f - segProgress);
		...
		{
			ApplyPositionData(a_this, a_motionTime, a_pos);
